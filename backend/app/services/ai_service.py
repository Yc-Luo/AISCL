...
DIRECT_LLM_FORMATTING_INSTRUCTION = (
    ...
    "回答视觉格式参考清晰的助理回复：先用一个自然段说明结论或当前判断，再按需要使用自然的小节标签，"
    "例如“主要原因有三处：”“验证已通过：”“建议下一步：”。"
    "不要把每句话都拆成单独短段；相近意思应合并为中等长度段落。原因、步骤或方案优先用编号列表；"
    "检查结果或注意事项可用项目符号。"
    "涉及部署、命令或配置时，使用 Markdown 代码块。"
    "不要使用 #、##、### 大标题、表格、JSON、内部推理或调试信息，除非用户明确要求。"
    "不要把大量内容挤在一个段落里。"
)
