        ...
        >
            <ReactMarkdown
                ...
                components={{
                    h1({ children }) {
                        return (
                            <h1 className="mb-2 mt-3 rounded-xl border-l-4 border-indigo-500 bg-indigo-50 px-3 py-2 text-base font-black leading-6 text-slate-900">
                                {children}
                            </h1>
                        )
                    },
                    h2({ children }) {
                        return (
                            <h2 className="mb-2 mt-3 rounded-xl border-l-4 border-indigo-400 bg-indigo-50/80 px-3 py-2 text-[15px] font-black leading-6 text-slate-900">
                                {children}
                            </h2>
                        )
                    },
                    h3({ children }) {
                        return (
                            <h3 className="mb-1.5 mt-3 rounded-lg bg-slate-100 px-3 py-1.5 text-sm font-black leading-6 text-slate-900">
                                {children}
                            </h3>
                        )
                    },
                    ...
                    ul({ children }) {
                        return <ul className="my-2 list-disc space-y-1 rounded-xl bg-slate-50 py-2 pl-7 pr-3">{children}</ul>
                    },
                    ol({ children }) {
                        return <ol className="my-2 list-decimal space-y-1 rounded-xl bg-slate-50 py-2 pl-7 pr-3">{children}</ol>
                    },
                    li({ children }) {
                        return <li className="pl-1 text-slate-700 marker:text-indigo-400">{children}</li>
                    },
                    blockquote({ children }) {
                        return (
                            <blockquote className="my-2 rounded-xl border-l-4 border-amber-300 bg-amber-50 px-3 py-2 text-slate-700">
                                {children}
                            </blockquote>
                        )
                    },
